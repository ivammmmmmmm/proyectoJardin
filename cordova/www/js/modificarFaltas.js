// Copiado desde /js/modificarFaltas.js (sin modificaciones de endpoints)
...
console.log('API_PREFIX establecido como:', API_PREFIX);
...
async function abrirModalModificar(alumnoId, fecha) {
    try {
        ...
        // Formatear fecha: si tiene T, es ISO (2024-01-15T00:00:00), si no, asumir YYYY-MM-DD
        ...
        try {
            ...
            console.error('Error parseando JSON:', e, responseText);
        ...
        alert('Error al cargar los datos de la falta: ' + error.message);
    }
}

// Eliminar falta usando falta_id
...
// Cargar razones de ausencia desde el servidor
async function cargarRazones() {
    try {
    const response = await fetch(API_PREFIX + 'obtenerRazones.php');
        if (!response.ok) {
...
// Mostrar/ocultar el selector de razón cuando cambia el estado
