      ...
      async function cargarDocentes() {
        try {
          ...
          const testConn = await fetchWithErrorHandling('./php/test_connection.php');
          console.log('Test de conexión:', testConn);

          // Si la conexión está bien, cargamos los docentes
          const data = await fetchWithErrorHandling('./php/verDocentes.php');
      ...
      function eliminarDocente(id) {
        if (confirm('¿Estás seguro de que deseas eliminar este docente?')) {
          ...
          fd.append('id', id);
          fetch(`./php/eliminarDocente.php?id=${encodeURIComponent(id)}`, {
            method: 'POST',
            body: fd
          })
          .then(async response => {
      ...
      async function mostrarSalas(docenteId) {
        ...
        try {
          ...
          const response = await fetch(`./php/obtenerSalas.php?tipo=con_docentes`);
          if (!response.ok) {
            throw new Error('Error en la respuesta del servidor: ' + response.status);
          }
          const text = await response.text();
      ...
      async function guardarModificacion() {
        try {
          ...
          const response = await fetch('./php/modificarDocente.php', {
            method: 'POST',
            body: formData
          });
